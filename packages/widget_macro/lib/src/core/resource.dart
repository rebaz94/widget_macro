...
import 'package:widget_macro/src/extensions/debounce.dart';
import 'package:widget_macro/src/extensions/value_notifier.dart';
...
/// {@template resource}
/// `Resources` are special `ValueNotifier`s designed specifically to handle Async
...
/// and __loading__.
///
/// Resources can be driven by a `source` value notifier that provides the query to an
...
/// again or subscribes again to the [stream].
/// {@endtemplate}
class Resource<T> extends ValueNotifier<ResourceState<T>> {
  /// {@macro resource}
  Resource(
    this.fetcher, {
    this.source,
    ...
    /// Whether to track the previous value of the resource, defaults to true.
    this.trackPreviousState,

    /// The debounce delay when the source changes, optional.
    this.debounceDelay,
  }) : useRefreshing = useRefreshing ?? true,
       stream = null,
       super(ResourceState<T>.loading()) {
    _resolveIfNeeded();
  }
  ...
  /// Whether to track the previous value of the resource, defaults to true.
  final bool? trackPreviousState;

  /// Reactive value notifier passed to the fetcher, optional.
  final ValueNotifier<Object?>? source;
  ...
  void Function()? _sourceDisposeObservation;
  ...
  ResourceState<T>? _previousValue;

  /// Updates the current resource state
  @override
  set value(ResourceState<T> state) {
    if (trackPreviousState == true) _previousValue = super.value;
    super.value = state;
  }

  /// The previous resource state
  ResourceState<T>? get previousValue {
    if (!_resolved) return null;
    return _previousValue;
  }

  // The stream transformed in a broadcast stream, if needed
  ...
  Future<void> _resolve() async {
    ...
    if (source != null) {
      ...
      source!.addListener(_sourceDisposeObservation!);
    }
  }
  ...
  void dispose() {
    ...
    if (_sourceDisposeObservation != null) {
      source?.removeListener(_sourceDisposeObservation!);
    }
  ...
  @override
  String toString() => '''Resource<$T>(value: $value, previousValue: $_previousValue)''';
}
...
sealed class ResourceState<T> {
  ...
  const factory ResourceState.ready(T data, {bool isRefreshing}) = ResourceReady<T>;
